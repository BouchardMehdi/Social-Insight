...
function fetchPage(offset = posts.offset) {
  posts.offset = offset
  posts.fetchPosts({
...
<template>
  <section class="page-stack">
    ...
    <section class="table-panel">
      ...
      <p v-if="!posts.items.length" class="empty-state">Aucun post ne correspond aux filtres</p>
    </section>

    <footer class="pagination-bar">
      <span>{{ posts.offset + 1 }}-{{ Math.min(posts.offset + posts.limit, posts.total) }} / {{ posts.total }}</span>
      <div>
        <button class="icon-button" :disabled="posts.offset === 0" @click="fetchPage(Math.max(0, posts.offset - posts.limit))">
          <ChevronLeft :size="18" />
        </button>
        <button class="icon-button" :disabled="posts.offset + posts.limit >= posts.total" @click="fetchPage(posts.offset + posts.limit)">
